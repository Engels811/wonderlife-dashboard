const servers={
  ...
  'WonderLife Routen':'1438476402277220364',
  'WonderLife Development':'1438326142846111917',
  'WonderLife Entbannung':'1438473736729923659'
};

async function load(){
  const box=document.getElementById("server-live");
  for(const name in servers){
    const guild=await discord(`users/@me/guilds`);
    const div=document.createElement("div");
    div.className="card";
    div.innerHTML=`<h3>${name}</h3><p>ID: ${servers[name]}</p><p>Status: Live</p>`;
    box.appendChild(div);
  }
}
window.onload=load;
